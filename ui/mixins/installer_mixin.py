...
class InstallerMixin:
    """Методы установки инструментов и сборки проектов."""
    ...
    def install_missing_tools(self):
        """Запускает установку недостающих инструментов вручную (последовательно, с запросом перезагрузки)."""
        missing = []
        if not self._is_tool_installed("git"):
            missing.append(("Git", self._get_git_url()))
        if not self._is_python_installed():
        ...
        def _run_installations():
            for name, url in missing:
                if ".NET SDK" in name:
                    success = self._install_sdk(url, blocking=True)
                else:
                    success = self._install_tool(name, url, blocking=True, ask_reboot=True)
                if not success:
                    self.log(f"❌ Установка {name} не удалась.", tag="error")
                    break
                time.sleep(1)

        threading.Thread(target=_run_installations, daemon=True).start()

    def _auto_install_sequence(self, tools):
        """Автоматическая последовательная установка инструментов (без запроса перезагрузки)."""
        for name, url in tools:
            if name == "Python":
                success = self._install_tool(name, url, blocking=True)
            elif name == "Git":
                success = self._install_tool(name, url, blocking=True)
            ...
            if not success:
                self.log(f"❌ Не удалось установить {name}. Прерывание автоустановки.", tag="error")
                break
            time.sleep(2)

    # ================== Универсальные методы установки ==================

    def _install_tool(self, name, url, ask_reboot=False, blocking=False):
        """
        Универсальный метод установки Python или Git.
        При blocking=True выполняется синхронно и возвращает bool.
        При blocking=False запускается в отдельном потоке (для GUI) и возвращает None.
        """
        if not self._is_admin():
            self.log("❌ Требуются права администратора для установки.", tag="error")
            if blocking:
                messagebox.showwarning("Нет прав администратора",
                                       "Для установки инструментов требуются права администратора.")
                return False
            else:
                self.after(0, lambda: messagebox.showwarning("Нет прав администратора",
                                                             "Для установки инструментов требуются права администратора."))
                return
        ...
        def _worker():
            try:
                ...
                if ask_reboot and name in ("Python", "Git"):
                    if blocking:
                        if messagebox.askyesno("Перезагрузка",
                                               f"Установка {name} завершена.\n"
                                               "Для обновления системного PATH может потребоваться перезагрузка.\n"
                                               "Перезагрузить сейчас?"):
                            self._restart_computer()
                    else:
                        self.after(0, lambda: messagebox.askyesno("Перезагрузка",
                                                                  f"Установка {name} завершена.\n"
                                                                  "Для обновления системного PATH может потребоваться перезагрузка.\n"
                                                                  "Перезагрузить сейчас?") and self._restart_computer())
    ...
    def _install_sdk(self, version, blocking=False):
        """
        Универсальный метод установки .NET SDK.
        При blocking=True выполняется синхронно и возвращает bool.
        При blocking=False запускается в отдельном потоке (для GUI) и возвращает None.
        """
        if not self._is_admin():
        ...
        self._safe_progress_config(mode="determinate")
        self._safe_progress_set(0)
        self._safe_speed_set(f"Загрузка SDK {version}...")

        dest_name = url.split("/")[-1]  # теперь имя файла корректное (win-x64, win-arm64)
        result = {"success": False}
    ...
    def _install_tool_blocking(self, name, url, ask_reboot=False):
        return self._install_tool(name, url, ask_reboot=ask_reboot, blocking=True)

    def _install_sdk_blocking(self, version):
        return self._install_sdk(version, blocking=True)

    def _download_and_run_installer(self, name, url, ask_reboot=False):
        """Асинхронный запуск установки Python/Git (для вызова из GUI)."""
        self._install_tool(name, url, ask_reboot=ask_reboot, blocking=False)

    def _download_and_install_sdk(self, version, build_name=None, build_path=None):
        """Асинхронный запуск установки .NET SDK (для вызова из GUI)."""
        self._install_sdk(version, blocking=False)

    # ================== Методы сборки проектов ==================
